...
- 还不是 git 仓库的话，先执行 **OBSync：初始化仓库** ——
  它会顺手建一份 `.gitignore`（排除 `.obsidian/workspace.json` 这类**每台设备各自维护**
  的文件，同步它们只会制造冲突）。已有 `.gitignore` 的话不会被覆盖
...
设置页「仓库同步」页底部有「**测试连接**」。它会依次检查
git 是否可用 → 库是否已初始化 → 是否配了远端 → 平台能否识别 → **真的连一次远端**，
并逐条给出结果。

最后一条是关键：**令牌填了不代表有效**，只有真的连一次才知道。
这个测试是只读的，不会改动任何东西。

但它**只验证「读取」**：推送权限、以及平台对凭据用户名的要求，
只有真正推送一次才能确认 —— 界面上也写明了这一点。
（实测：Gitee 对凭据用户名的校验只在推送路径上执行，`ls-remote` 看不到，
所以这个测试报「全部通过」时并不代表推送一定成功。）
