...
| 用户的话 | 落地 |
| --- | --- |
| 指定图片文件夹 | `images.folders` —— **唯一的边界**，只有落在这里的文件会被同步 |
| 双副本架构、本地和云端各存一份 | 镜像：每一轮把两边缺的补上（上传 + 下载） |
| ~~双向删除同步~~ → 删本地时问一句 | 见 8.3（次日撤掉了双向删除） |
| 在笔记内直接裁剪 / 压缩 | 阅读视图的悬浮工具条 + 命令面板 + 文件右键菜单 |

### 8.2 明确不做的

- **不做双向删除**（2026-09-23 撤掉）：判断「一边少了文件 = 用户删的」本质上做不准，
  错的代价是删掉用户没打算删的东西。详见 HANDOVER 五点九。
- **不做多桶 / 多账号**：一组 R2 配置服务整个库。
- **不碰 `.gitignore`**：图片文件夹通常同时在 git 仓库里，两条链路各管各的
  （git 管版本历史，R2 管「不占仓库体积、能被外链引用」）。
- **不做批量格式转换**：编辑是逐张的 —— 用户说的是「在笔记内直接」，那是一个
  看着图片做决定的动作，不是一个批处理任务。
- **不做 Live Preview 里的工具条**：CM6 的图片 widget 没有公开钩子，挂上去要靠猜
  内部结构，Obsidian 一升级就断。编辑模式的入口给的是命令与右键菜单。

### 8.3 删除语义（与「双向同步」差别最大的一处）

删除**不在同步流程里**，而是用户主动发起的一次交互：

```
用户在库里删掉一张图片
  → vault.on("delete")
  → 攒批 400ms（一次删十张只问一次）
  → 云端有备份吗？没有就不问（这是绝大多数情况）
  → 弹窗：「要从云端也删除这张图片吗？」
  → 删：deleteRemoteBackup()   ／   留：keepRemoteBackup()（记墓碑）
```

「留」要记墓碑，否则下一轮同步会把那张图下载回来 —— 看起来像「删除没生效」。

### 8.4 与其它模块的关系

- **与 `host/` 无关**：R2 走自己的 S3 客户端，不复用 GitHub / Gitee 那层。
  但 HTTP 走同一个 `host/http.ts`（重试、超时、脱敏是同一套行为）。
- **凭据走 `core/secretStore`**（键 `r2`）：Secret Access Key 绝不进 `data.json`，
  与平台令牌同一条路径。`Access Key ID` 留在设置里 —— 它是标识不是秘密。
- **移动端也装**：只用 `requestUrl` + vault 文件 API，两者在移动端都有。
  代价是这个模块的整条静态导入链里不能出现任何依赖 Node 的东西。
