...
**五、「提交并推送」（2026-09-19 追加）。** 用户接着要求加一个按钮。它就是
`sync()` **去掉拉取**：`commitAndPush()` = `doCommitAll()` → `doPush()`。

为什么值得单列一个动作，而不是「想推就用立即同步」：**拉取会动工作区**
（merge 可能产生合并提交、rebase 会改写本地提交、reset 更是丢弃本地提交），
而有些人要的只是「把我的改动变成提交再送上去，别来合并我的文件」。
代价写在提示里：远端真有新提交时推送被拒（`PushRejectedError`）——
这是**故意**的，报错比悄悄合并安全。

两处实现细节值得记住：

- **「没有更改可提交」不能提前返回。** `commitAndPush` 里的提交是尽力而为的：
  工作区干净不代表没有要推的东西（上次的提交可能还没推上去）。所以不能像
  `commitAll` 那样在 `nothing-to-commit` 时收工 —— 用例钉着这一条。
- **活动态按阶段走**（committing → pushing → idle），不能整条链路都显示「正在提交」。

命令面板也加了同名命令（`OBSync：提交并推送（不拉取）`），与视图里的按钮同一入口。

**为什么默认链路是「提交 → 拉取 → 推送」（用户问的）。**
...
- **代价**：拉取会动工作区，而策略可配（`merge` 默认 / `rebase` / `reset`）。
  **`reset` 会丢弃本地提交**（`resetToRemote` 先 stash 保护未跟踪与未提交改动，再
  `reset --hard`），选它的人要清楚这一点 —— 这也是「提交并推送」存在的意义：
  不想让拉取碰工作区时，跳过它，代价只是远端有新提交时推送被拒。
