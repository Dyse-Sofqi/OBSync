...
写它（`recordItem`），`reinstall()` 也读它（重装装回那一版）。但界面上**只有**
...
`switchVersion` 走 `install()`（**不是** `reinstall()`），因为要装的正是用户刚选的
那个 tag；`install` 会把它写回 `requestedVersion`。于是三条现成的语义一起成立：

| 用户动作 | 结果 |
| --- | --- |
| 选一个具体 tag | 装它，且**钉住**：此后「重装」装回这一版 |
...
**必须显示**：这个状态只存在于 `data.json` 里，而它的后果是「重装会装回旧版」——
不显示的话，用户看到版本号是旧的，分不清那是自己选的还是更新失败留下的。
...
- 换来源走 `confirmMirror`：记录的主来源换成它、原来源留在 `origin`，此后下载、
  更新检查、重装都按新来源走（列表两行地址都还在，见 `mirrorProvenance.test.ts`）；
...
各加一个可选参数。行内动作统一走 `runWithProgress`（`TrackedItemsList.ts`），
它负责转圈、进度、错误提示三件事，避免四条动作各写一遍。
