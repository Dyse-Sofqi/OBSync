 ...
 * 「提交并推送」= 立即同步**去掉拉取**。
 *
 * 为什么两个都要有：立即同步会先拉取，而拉取**会动工作区**（可能合并、可能冲突）；
 * 有些人就是不想让它动。代价是远端真有新提交时推送会被拒 —— 这是故意的，
 * 报错比悄悄合并安全。
 */
describe("commitAndPush（提交并推送，不拉取）", () => {
    it("先提交再推送，**完全不碰远端拉取**", async () => {
        const git = new FakeGit();
        const fake = createFakeApp();
        const { service } = makeService(git, fake);
        git.unstaged = ["a.md"];
        ...
        const outcome = await service.commitAndPush();
    ...
    it("提交在推送之前（顺序错了会推上去一个空提交）", async () => {
        const git = new FakeGit();
        const fake = createFakeApp();
        const { service } = makeService(git, fake);
        git.unstaged = ["a.md"];
        ...
        const commitIndex = git.calls.findIndex((call) => call.startsWith("commit:"));
        expect(commitIndex).toBeGreaterThan(-1);
        expect(commitIndex).toBeLessThan(git.calls.indexOf("push"));
    });

    it("活动态按阶段走：提交 → 推送 → 恢复（不能整条链路都显示「正在提交」）", async () => {
        const git = new FakeGit();
        const fake = createFakeApp();
        const { service, activities } = makeService(git, fake);
        git.unstaged = ["a.md"];
        ...
        expect(activities).toEqual(["committing", "pushing", "idle"]);
    });

    it("没有更改可提交时，仍然把已有的提交推上去", async () => {
        // 「提交并推送」里的提交是**尽力而为**：没有新改动不代表没有要推的东西
        // （上次提交可能还没推上去）。这里不能被 nothing-to-commit 提前打断。
        const git = new FakeGit();
        const fake = createFakeApp();
        const { service } = makeService(git, fake);
        git.ahead = 2;
    ...
    it("冲突未解决时拒绝提交，也**不推送**（不能把冲突标记推上去）", async () => {
        const git = new FakeGit();
        const fake = createFakeApp();
        const { service } = makeService(git, fake);
        git.conflicted = ["notes/会打架.md"];
        ...
        await expect(service.commitAndPush()).rejects.toBeInstanceOf(ConflictError);

        expect(git.calls).not.toContain("push");
    });

    it("用户主动发起时会说明「没有需要推送的内容」", async () => {
        const git = new FakeGit();
        const fake = createFakeApp();
        const { service, notices } = makeService(git, fake);
        git.ahead = 0;

        await service.commitAndPush({ announceIfUpToDate: true });
